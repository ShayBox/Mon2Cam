...
export default async function(logger:Logger, options:Options) {
    ...
    async function createNullSink(name:string, description:string): Promise<Number> {
        return new Promise(async resolve => {
            let cmd = await exec(`pactl load-module module-null-sink sink_name="${name}" sink_properties=device.description="${description}"`, {output:OutputMode.Capture});
            if(cmd.status.success) {
    ...
    async function createCombinedSink(name:string, description:string, slaves:string[]): Promise<Number> {
        return new Promise(async resolve => {
            ...
            let cmd = await exec(`pactl load-module module-combine-sink sink_name="${name}" slaves="${slaves.join()}" sink_properties=device.description="${description}"`, {output:OutputMode.Capture});
